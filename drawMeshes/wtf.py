...
def drawSquareMesh(Nmode):
    ...
    scale = 2
    turtle.setup(height/scale, height/scale)
    ...
    if(startValue == 0):
        ...
        for i in range(1,len(topPoints)-1,2):
            t.goto(topPoints[i].x,topPoints[i].y)
            t.pendown()
            t.goto(topPoints[i+1].x,topPoints[i+1].y)
            t.penup()

        for i in range(0,len(botPoints)-1,2):
            t.goto(botPoints[i].x,botPoints[i].y)
            t.pendown()
            t.goto(botPoints[i+1].x,botPoints[i+1].y)
            t.penup()
    ...
    if startValue == 1:
        ...
        t.goto(topPoints[0].x,topPoints[0].y)
        t.pendown()
